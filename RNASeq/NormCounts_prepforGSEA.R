...
hh1 <- readRDS('./results/DESeq2/20230501_H_DEout.rds')
cc1 <- readRDS('./results/DESeq2/20230501_C_DEout.rds')

hh2 <- readRDS('./results/DESeq2/20230501_H_dds.rds')
cc2 <- readRDS('./results/DESeq2/20230501_C_dds.rds')

hh2 <- estimateSizeFactors(hh2)
normcounts <- as.data.frame(counts(hh2, normalized=TRUE))
normcounts$Gene.stable.ID <- rownames(normcounts)
normcounts1 <- merge(hh, normcounts, by = 'Gene.stable.ID')

write.csv(normcounts1, './results/DESeq2/20241123_hhnormcounts.txt')

cc2 <- estimateSizeFactors(cc2)
normcounts <- as.data.frame(counts(cc2, normalized=TRUE))
normcounts$Gene.stable.ID <- rownames(normcounts)
normcounts1 <- merge(cc, normcounts, by = 'Gene.stable.ID')

write.csv(normcounts1, './results/DESeq2/20241123_ccnormcounts.txt')
